...
target_population <- "/CD45+/Single Cells/Single Cells 2/Viable Cells/Not CD3+ CD19+/Not CD66b+ "

exclude_keywords <- c("Unstained", "Compensation", "LiveDeadFixableBlue", "Control", "FMO")
...
for(sample_type in c("Innate", "Adaptive")){
  ...
  flowjo_counts_cache <- extract_all_flowjo_counts(gs_dirs = gs_dirs, 
                                                   flowjo_root = flowjo_root, 
                                                   sample_type = sample_type,force = T)
  
   
  # STEP 1: Collect statistics and plot info
  cat("\nSTEP 1: Collecting data and calculating marker statistics...\n")
  gs <- load_gs(gs_dirs[1])
  paths = gs_get_pop_paths(gs, path='full')
  paths= paths[-1]
  for (target_population in paths[2]){
    all_stats <- list()
    all_plot_info <- list()
    all_marker_stats <- list()
    
    for(gs_dir in gs_dirs[1]){
      ...
      tryCatch({
        ...
        for(sample_idx in seq_len(nrow(stats))){
          ...
          marker_stats <- extract_marker_stats(gs, sample_gs, target_population, trans)
    ...
    if(is_empty(all_stats)) next()
    save(file = paste0("afterStep1.",sample_type, ".RData"), list = ls())
    # cp = load(paste0("afterStep1.",sample_type, ".RData"))
    combined_stats <- do.call(rbind, all_stats)
    if(nrow(combined_stats) < 1) next()
    combined_stats <- combined_stats[order(-combined_stats$Count), ]
    ...
    # Sort plot info by count
    counts <- vapply(all_plot_info, function(x) x$count, numeric(1))
    all_plot_info <- all_plot_info[order(counts, decreasing = TRUE)]
    
    # STEP 2: Aggregate marker statistics
    cat("\nSTEP 2: Aggregating marker statistics across all samples...\n")
    agg_stats <- aggregate_marker_stats(all_marker_stats)
    
    if (is.null(agg_stats)) {
      message("Could not aggregate statistics, skipping...")
      next()
    }
    ...
    pdf_file <- file.path(flowjo_root, sprintf("%s_%s_sorted.pdf", sample_type, make.names(target_population)))
