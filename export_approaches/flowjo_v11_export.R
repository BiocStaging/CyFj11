...
# Export with the original workspace data to preserve structure
# Extract the JSON data correctly
template_json_data <- ws$json[[1]]
workbench_data <- list(preferences = ws$preferences, workbench = ws$workbench)

{
  ...
  result <- toc()
  elapsed <- result$toc - result$tic
  cat(sprintf("\nCompleted in: %.1f seconds (%.2f minutes)\n", elapsed, elapsed/60))
  pretty_print_flowjo("test.3.flowjo")
}

convert_json_to_flowjo(json_path = "test.3_pretty.json",
                       output_path = "test.4.flowjo",
                       original_flowjo_path = "test.3.flowjo")
