...
#' Get Inverse Transform Function for a Channel
#'
#' Returns a function that maps transformed coordinates back to raw data space.
#' Used by FlowJo v10 export since v10 XML stores coordinates in raw space.
#'
#' @param gh GatingHierarchy object
#' @param channel Channel name (e.g., "FITC-A")
#' @return A function that takes transformed values and returns raw values,
#'         or identity function if no transform exists
#' @keywords internal
# get_inverse_transform_function <- function(gh, channel) {
#   
#   # Identity function as fallback
#   identity_fn <- function(x) x
#   
#   if (is.null(gh)) return(identity_fn)
#   
#   trans_list <- tryCatch(
...
#   )
#   
#   if (is.null(trans_list) || !channel %in% names(trans_list)) {
#     return(identity_fn)  # Linear/no transform — coordinates stay as-is
#   }
#   
#   trans <- trans_list[[channel]]
#   trans_type <- attr(trans, "type")
#   
#   if (is.null(trans_type)) return(identity_fn)
#   
#   # Try to get the inverse transform directly from flowWorkspace
#   inv_fn <- tryCatch({
#     gh_get_transformations(gh, channel = channel, inverse = TRUE)
#   }, error = function(e) NULL)
#   
#   if (!is.null(inv_fn) && is.function(inv_fn)) {
#     return(inv_fn)
#   }
#   
#   # Fallback: build inverse from transform spec
#   spec <- get_transform_spec(gh, channel)
#   if (is.null(spec)) return(identity_fn)
#   
...
#          "Biex" = {
#            tt <- tryCatch(
#              create_biexponential_transform(spec = spec),
#              error = function(e) NULL
#            )
#            if (!is.null(tt) && !is.null(tt$inverse)) tt$inverse else identity_fn
#          },
#          
#          "Log" = ,"logtGml2" =, "flowJo_log" = {
#            tt <- tryCatch(
#              create_log_transform(spec = spec),
#              error = function(e) NULL
#            )
#            if (!is.null(tt) && !is.null(tt$inverse)) tt$inverse else identity_fn
#          },
#          
#          "Arcsinh" = {
#            # Inverse of asinh(x * cofactor) is sinh(y) / cofactor
#            # For GML2 asinh: f(x) = asinh(x * sinh(M * ln(10)) / T) / (M * ln(10))
#            # Inverse: f^-1(y) = T * sinh(y * M * ln(10)) / sinh(M * ln(10))
#            T_val <- spec$T %||% 262144
#            M_val <- spec$M %||% 4.5
#            A_val <- spec$A %||% 0
#            
#            function(y) {
#              T_val * sinh(y * M_val * log(10)) / sinh(M_val * log(10))
#            }
#          },
#          
#          "Logicle" = {
#            # Use flowCore's logicleTransform inverse if available
#            tryCatch({
#              lt <- flowCore::logicleTransform(
#                T = spec$T, W = spec$W, M = spec$M, A = spec$A
#              )
#              flowCore::inverseLogicleTransform(lt)@.Data
#            }, error = function(e) {
#              warning("Could not create logicle inverse for ", channel)
#              identity_fn
#            })
#          },
#          
#          # Default
#          {
#            warning("No inverse available for transform type: ", spec$transformType,
#                    " on channel ", channel)
#            identity_fn
#          }
#   )
